...
`HHRU_SECRET_KEY=(ваш ключ)` и `HHRU_ACCESS_TOKEN=(ваш access_ключ)`;\
 - по сайту [SuperJob.ru](https://superjob.ru/)
   - для корректной работы `.env` задать в файле `.env` наименование переменной окружения схожей в имеющемся коде, например, `superjob_client_secret=(ваш ключ)`,
`superjob_access_token=(ваш access_ключ)`;
